...
void wa1470dem_init(uint32_t preambule)
{
	...
	if(preambule) wa1470dem_set_preambule((uint8_t *)&preambule);
...
static void  wa1470dem_process_messages(struct scheduler_desc *desc)
{
	...
	if(wa1470_hal->__wa1470_data_received)
	{
		for(uint8_t i = 0; i != tmp_dem_mess_received; i++)
		{
			...
			sprintf(wa1470_log_string + strlen(wa1470_log_string), " IT crypto=%3d FREQ=%2d", tmp_dem_mas[i].packet.iter, tmp_dem_mas[i].freq&0x1f);
#endif
...
static void wa1470dem_update_noise(struct scheduler_desc *desc)
{
	...
	if(dem_rx_enabled) wa1470_scheduler->__scheduler_add_task(desc, 0, RELATIVE, MILLISECONDS(DEM_NOISE_TICK));
}
